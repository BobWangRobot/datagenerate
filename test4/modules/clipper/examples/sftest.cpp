...
int main( int argc, char** argv )
{
  ...
  for ( HKL_info::HKL_reference_index ih = hkls.first(); !ih.last(); ih.next() ) {
    std::cout << ih.hkl().format() << " " << fphi4[ih].f() << " " << fphi5[ih].f() << "\n";
    std::cout << "                        " << clipper::Util::rad2d(fphi4[ih].phi()) << " " << clipper::Util::rad2d(fphi5[ih].phi()) << "\n";
  }
  // */
  ...
  */
  ...
  for ( int i = 0; i < 50; i++ ) {
    ...
    sf2.rho_grad( c2, g );
    std::cout << (sfx.rho(c2)-sf2.rho(c2))/0.001 << ":" << g[0] << "\t" << (sfy.rho(c2)-sf2.rho(c2))/0.001 << ":" << g[1] << "\t" << (sfz.rho(c2)-sf2.rho(c2))/0.001 << ":" << g[2] << "\t" << (sfo.rho(c2)-sf2.rho(c2))/0.001 << ":" << g[3] << "\t" << (sfu.rho(c2)-sf2.rho(c2))/0.001 << ":" << g[4] << "\n";
  }
