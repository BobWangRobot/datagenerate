...
def format_HELIX_records_from_AEV(aev_values_dict):
  ...
  """
  threshold1 = [2.48, 0.9, 5]
  threshold2 = [2.38, 0.9, 2]
  ...
  for key,value in aev_values_dict.items():
    ...
    if resi == 'PRO':
      ...
      if start == [] and value['B'] > cut_num[1]:
        start = key
      elif start and value['E'] > cut_num[1]:
        end = key
      if start and end and value['E'] < 0.7:
        #there are no M or B behind E.
      ...
      if value['E'] == None or value['B'] == None:
        # ignore strat and end value None
        if start == [] and value['B'] > cut_num[1]:
          start = key
        elif start and value['E'] > cut_num[1]:
          end = key
        if start and end and (value['M'] == None or value['E'] < 0.7):
          length = int(end.split()[-1]) - int(start.split()[-1]) + 1
          if length > cut_num[2]:
            i += 1
            fmt = "HELIX   {0:>2}  {0:>2} {1:>}  {2:>}   {3:>36}"
...
class AEV(object):
  ...
  def keep_protein_altloc(self):
    ...
    ss = " or ".join(aa_codes)
    self.hierarchy.select(asc.selection(ss))
    self.hierarchy.remove_alt_confs(always_keep_one_conformer=True)
    if self.hierarchy.atoms_size() == 0:
      return None
    else:
...
#      self.chain_hierarchy.write_pdb_file(file_name=str(egg)+"_test.pdb")
#      for atom in self.chain_hierarchy.atoms():
