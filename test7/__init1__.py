...
def format_HELIX_records_from_AEV(aev_values_dict, cc_cutoff):
  ...
  for key,value in aev_values_dict.items():
    if start==[] and value['B'] > cc_cutoff and value['M'] > cc_cutoff:
      start = key
    elif start and value['E'] > cc_cutoff:
      end = key
      length = int(end.split()[-1]) - int(start.split()[-1]) + 1
    if start and end and value['E'] < cc_cutoff and value['M'] < cc_cutoff and length > 5:
      i += 1
      fmt = "HELIX   {0:>2}  {0:>2} {1:>}  {2:>}   {3:>36}"
      result.append(fmt.format(i, start, end, length))
      start = []
      end = []
    if value['B'] == None or value['E'] == None or value['M'] == None:
      start = []
      end = []
  return result
