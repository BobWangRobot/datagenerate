...
def get_AEV_result(pdb_inp,run_list, cut_num):
  ...
  CC_value = aev.compare(a, run_list)
  recs = aev.format_HELIX_records_from_AEV(CC_value, cut_num)
  return recs

def cal(filename, run_list):
  ...
  v2 = get_AEV_result(pdb_inp, run_list, cut_num=[0.95, 6])
  return v1, v2

def run():
  t0 = time.time()
  Max = [0, []]
  path = '/home/bob/datagenerate/test13/phi_psi_test_set/high'
  for a in range(80, 101):
    for b in range(a, 101):
      AEV_method = flex.double()
      records = flex.double()
      c = 100 - b
      run_list = [(b-a)/100, a/100, c/100]
      print(run_list)
      for root, dirs, files in os.walk(path):
        for file in files:
          ...
          if 'pdb' in filename:
            print(filename)
            value_result = cal(filename, run_list)
            records.extend(value_result[0])
            AEV_method.extend(value_result[1])
      AEV_CC = flex.linear_correlation(x=records, y=AEV_method).coefficient()
      if AEV_CC > Max[0]:
        Max[0] = AEV_CC
        Max[1] = run_list
      print("AEV_CC:", AEV_CC)
